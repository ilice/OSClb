...
sed -i "s/__api.opensmartcountry.com__/$API_OPENSMARTCOUNTRY_DOMAIN/g" /etc/nginx/conf.d/osc.conf
...
if [ -z "$OPENSMARTCOUNTRY_DOMAIN" ]; then
  ...
  sed -i "s/__api.opensmartcountry.com__/$API_OPENSMARTCOUNTRY_DOMAIN/g" /etc/nginx/osc-secure.conf
